...
import com.netflix.nicobar.core.module.jboss.*;
...
/**
 * Top level API for loading and accessing scripts. Builds and maintains interdependent script
 * modules. Performs coordination between components neccessary for finding, compiling and loading
 * scripts and notifying event listeners.
 *
 * Listeners can be added at any time during the repositories lifecycle, but if they are added at
 * construction time, they are guaranteed to receive the events associate with the loading of
 * archives, while those that are added later will only get events generated after the listener was
 * added.
 *
...
public class ScriptModuleLoader {
    ...
    protected final Set<ScriptModuleListener> listeners = Collections
            .newSetFromMap(new ConcurrentHashMap<ScriptModuleListener, Boolean>());
    ...
    public synchronized void updateScriptArchives(Set<? extends ScriptArchive> candidateArchives) {
        ...
        for (ScriptArchive scriptArchive : candidateArchives) {
            ...
            if (createTime < latestCreateTime) {
                notifyArchiveRejected(scriptArchive,
                        ArchiveRejectedReason.HIGHER_REVISION_AVAILABLE, null);
                continue;
            }

            // create the new revisionIds that should be used for the linkages when the new modules
            // are defined.
            ModuleIdentifier newRevisionId = JBossModuleUtils
                    .createRevisionId(scriptModuleId, updateNumber);
            ...
            archiveDependencies
                    .put(scriptModuleId, scriptArchive.getModuleSpec().getModuleDependencies());
        }
        ...
        DirectedGraph<ModuleId, DefaultEdge> candidateGraph = jbossModuleLoader
                .getModuleNameGraph();
        ...
        while (!leaves.isEmpty()) {
            for (ModuleId scriptModuleId : leaves) {
                ...
                try {
                    ...
                    notifyArchiveRejected(scriptArchive, ArchiveRejectedReason.ARCHIVE_IO_EXCEPTION,
                            ioe);
                }

                try {
                    moduleSpec = createModuleSpec(scriptArchive, candidateRevisionId,
                            updatedRevisionIdMap, moduleCompilationRoot);
                } catch (ModuleLoadException e) {
                    logger.error("Exception loading archive " + scriptArchive.getModuleSpec()
                                                                             .getModuleId(), e);
                    notifyArchiveRejected(scriptArchive, ArchiveRejectedReason.ARCHIVE_IO_EXCEPTION,
                            e);
                    continue;
                ...
                try {
                    ...
                    Files.walkFileTree(moduleCompilationRoot, new SimpleFileVisitor<Path>() {
                        public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
                            ...
                            if (relativePath.endsWith(".class")) {
                                String className = relativePath.replaceAll(".class", "")
                                                               .replace("/", ".");
                    ...
                    for (String loadClass : classesToLoad) {
                        Class<?> loadedClass = jbossModule.getClassLoader()
                                                          .loadClassLocal(loadClass, true);
                        if (loadedClass == null) {
                            throw new ScriptCompilationException(
                                    "Unable to load compiled class: " + loadClass);
                        }
                    }
                } catch (Exception e) {
                    // rollback
                    ...
                    if (candidateArchives.contains(scriptArchive)) {
                        ...
                        notifyArchiveRejected(scriptArchive, ArchiveRejectedReason.COMPILE_FAILURE,
                                e);
                    }
                    if (jbossModule != null) {
                ...
                JBossScriptModule scriptModule = new JBossScriptModule(scriptModuleId, jbossModule,
                        scriptArchive);
                ...
                Set<ModuleId> dependents = GraphUtils
                ...
                for (ModuleId dependentScriptModuleId : dependents) {
                    if (!archivesToCompile.containsKey(dependentScriptModuleId)) {
                        ScriptModule dependentScriptModule = loadedScriptModules
                                .get(dependentScriptModuleId);
     ...
     * @param moduleCompilationRoot a path to a directory that will hold compiled classes for this
     *                              module.
     */
    protected ModuleSpec createModuleSpec(ScriptArchive archive,
        ...
        Set<String> appImportFilterPaths = new HashSet<>();
        if (appPackagePaths != null && !appPackagePaths.isEmpty()) {
            appImportFilterPaths.addAll(appPackagePaths);
        }
        if (archiveSpec.getAppImportFilterPaths() != null) {
            appImportFilterPaths.addAll(archiveSpec.getAppImportFilterPaths());
        }
        JBossModuleUtils.populateModuleSpecWithAppImports(moduleSpecBuilder, appClassLoader, appImportFilterPaths);
        // JBossModuleUtils.populateModuleSpecWithAppImports(moduleSpecBuilder, appClassLoader,
        //         archiveSpec.getAppImportFilterPaths() == null ? appPackagePaths
        //                 : archiveSpec.getAppImportFilterPaths());

        // Allow compiled class files to fetched as resources later on.
        JBossModuleUtils
                .populateModuleSpecWithCompilationRoot(moduleSpecBuilder, moduleCompilationRoot);

        try {
            // Populate the modulespec with the scriptArchive dependencies
            Set<ModuleId> moduleDependencies = archiveSpec.getModuleDependencies();
            if (moduleDependencies != null) {
                for (ModuleId dependencyModuleId : moduleDependencies) {
                    ScriptModule dependencyModule = getScriptModule(dependencyModuleId);
                    Set<String> exportPaths = dependencyModule.getSourceArchive().getModuleSpec()
                                                              .getModuleExportFilterPaths();

                    JBossModuleUtils.populateModuleSpecWithModuleDependency(moduleSpecBuilder,
                            archiveSpec.getModuleImportFilterPaths(), exportPaths,
                            moduleIdMap.get(dependencyModuleId));
                }
            }
        } catch (Throwable e) {
            throw new ModuleLoadException(
                    "Populate the modulespec with the scriptArchive dependencies occurred error.",
                    e);
        }
     ...
     */
    protected void compileModule(Module module, Path moduleCompilationRoot)
        ...
        if (moduleClassLoader instanceof JBossModuleClassLoader) {
            ...
            if (candidateCompilers.size() == 0) {
                throw new ScriptCompilationException(
                        "Could not find a suitable compiler for this archive.");
            }
     ...
     */
    public synchronized void addCompilerPlugin(ScriptCompilerPluginSpec pluginSpec)
        ...
        Map<ModuleId, ModuleIdentifier> latestRevisionIds = jbossModuleLoader
                .getLatestRevisionIds();
        JBossModuleUtils
                .populateCompilerModuleSpec(moduleSpecBuilder, pluginSpec, latestRevisionIds);
        ...
        if (providerClassName != null) {
            ...
            try {
                ...
                ScriptCompilerPlugin pluginBootstrap = (ScriptCompilerPlugin) compilerProviderClass
                        .newInstance();
                Set<? extends ScriptArchiveCompiler> pluginCompilers = pluginBootstrap
                        .getCompilers(pluginSpec.getCompilerParams());
     ...
     *
     * @return immutable view of the Map that retains the script modules. Map ModuleId the loaded
     * ScriptModule
     */
    ...
    /**
     * Add listeners to this module loader. Listeners will only be notified of events that occurred
     ...
     */
    protected void notifyModuleUpdate(@Nullable ScriptModule newModule,
                                      @Nullable ScriptModule oldModule) {
     ...
     */
    protected void notifyArchiveRejected(ScriptArchive scriptArchive,
    ...
    public static class Builder {
         ...
         *
         * @param incomingPaths a set of / separated package paths. No wildcards. e.g. Specifying
         *                      com/foo/bar/baz implies that all classes in packages named
         *                      com.foo.bar.baz.* will be visible to loaded modules.
         */
